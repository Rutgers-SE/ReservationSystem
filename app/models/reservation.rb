...
class Reservation < ActiveRecord::Base
  ...
  def calculate_cost_in_pennies(price_in_pennies)
    (price_in_pennies * hours).to_i
  end

  class << self
    def remote_space_check(reservation)
      # for the final project, we need to make this connection secure
      response = RestClient.get "http://localhost:9001/api/asdf/b/checkreservation", accept: :json, params: {
    ...
    def remote_reserve_space(res)
      response = RestClient.get "http://localhost:9001/api/garage/b/setreservation", accept: :json, params: {
        start: res.start,
        finish: res.finish
      }
